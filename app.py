...
id = st.number_input(label='Nhập ID nhà hàng', value=None, placeholder="Type a number...", step=1, min_value=1, max_value=1622)
...
if st.button('Submit'):
    ...
    st.markdown("<h3 style='text-align: center;'> Đánh giá của bạn là: " + str(model.predict([comment])[0]) + "</h1>", unsafe_allow_html=True)
    ...
    # pie chart
    fig_pie = [go.Pie(labels=res[0], values=res[1], marker={'colors': ['#1f77b4', '#ff7f0e', '#2ca02c']})]
    fig_pie = go.Figure(fig_pie)
    fig_pie.update_layout(title={'text':'Tỉ lệ các loại đánh giá về nhà hàng', 'font': {'size': 30}})
    ...
    plt.title('Wordcloud các đánh giá tiêu cực\n\n', fontsize=20, fontweight='bold')
    plt.axis('off')
    ...
    plt.title('Wordcloud các đánh giá tích cực\n\n', fontsize=20, fontweight='bold')
    plt.axis('off')
